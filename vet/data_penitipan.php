...
<body>
	...
	$query2= mysqli_query($connect, "SELECT * FROM data_penitipan where id_penitipan='$id_penitipan'");
	$query3= mysqli_query($connect, "SELECT * FROM data_checkup where id_penitipan='$id_penitipan' ORDER BY id_dataCheckUp DESC");
	...
	?>
			...
			<div style="color:white; text-align:left;">
				<h6><?= ": ".$id_penitipan;?></h6>
				<h6><?= ": ".date('d F Y',strtotime($data2['tanggal_masuk']));?></h6>
				<h6><?= ": ".date('d F Y',strtotime($data2['tanggal_keluar']))?></h6>
			</div>
			</div>
		</div>
	</div>
	...
	<?php }?>
		...
		?>
			...
			<div class="row">
				...
				<div class="col-3">
					<?php if (!empty($_SESSION['id_dokter'])) { ?>
						<a type="button" href="inputCheckUp.php?id_hewan=<?= $id_hewan?>&id_penitipan=<?=$id_penitipan;?>&id_dokter=<?=$id_dokter;?>&message=penitipan" class="btn btn-dark btn-lg btn-merkcolor btn-sm" style="align:right">Tambah</a>
					<?php } ?>
				</div>
			</div>
